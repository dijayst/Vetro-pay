...
export default function TvDstv() {
  const provider = "StarTimes";
  const product = "STARTIMES";
  const [productCode, setProductCode] = useState("");
  const [startimesLoadedSubscriptionTypes, setStartimesLoadedSubscriptionTypes] = useState({
    data: [
      {
  ...
  useEffect(() => {
    dispatch(getTvBouquets("startimes"));
  }, []);
    ...
    if (prevGetTvBouquetsData) {
      if (prevGetTvBouquetsData.length !== getTvBouquetsData.length) {
        if (getTvBouquetsData[getTvBouquetsData.length - 1]["status"] == "success") {
          setStartimesLoadedSubscriptionTypes(getTvBouquetsData[getTvBouquetsData.length - 1]);
  ...
  const onChangeBouqet = (selectedValue) => {
    ...
    Object.keys(startimesLoadedSubscriptionTypes.data).map((value, index) => {
      const plans = startimesLoadedSubscriptionTypes.data[value];
  ...
  const onChangeBouquetDuration = (selectedValue) => {
    ...
    Object.keys(startimesLoadedSubscriptionTypes.data[tempProductIndex]).map((value, index) => {
      const availableOptions = startimesLoadedSubscriptionTypes.data[tempProductIndex][value];
  ...
  const verifySmartCard = () => {
    if (smartCardNumber !== "" && smartCardNumber.length >= 10) {
      ...
      dispatch(getCustomer("startimes", smartCardNumber));
    }
  };
    ...
    if (transactionPin != "") {
      const transactionDetails = {
        ...
        details: {
          ...
          subscription: startimesLoadedSubscriptionTypes.data[tempProductIndex]["name"],
              ...
              <View style={styles.innerContainer}>
                ...
                <View style={styles.picker}>
                  ...
                  >
                    ...
                    {startimesLoadedSubscriptionTypes.data.map((value, index) => {
                ...
                <View style={styles.picker}>
                  ...
                  >
                    <Picker.Item label={"Select"} value="" />
                    {startimesLoadedSubscriptionTypes.data[tempProductIndex]["availablePricingOptions"].map((value, index) => {
          ...
          <View style={styles.container}>
            <View style={styles.innerContainer}>
              ...
              <View style={styles.transactionDetailsRow}>
                ...
                <AppText>{startimesLoadedSubscriptionTypes.data[tempProductIndex]["name"]}</AppText>
              </View>
          ...
          <View style={styles.container}>
            <View style={styles.innerContainer}>
              ...
              <AppText styles={{ fontSize: 15, marginTop: 16, textAlign: "center" }}>
                Your Startimes subscription of NGN {totalAmount} to {smartCardNumber} was Successful
              </AppText>
