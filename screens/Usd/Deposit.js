...
export default function Deposit({ navigation }) {
  ...
  const usdDepositResponse = useSelector(
    (state) => state.blockchain.depositusd
  );
  ...
  useEffect(() => {
    if (
      userUsdDepositDetails.payload.amount != "" &&
      usdDepositResponse.length &&
      usdDepositResponse.length != prevUsdDepositResponse?.length
    ) {
      if (
        usdDepositResponse[usdDepositResponse.length - 1]?.status == "success"
      ) {
        setDisplaySpinner(false);
        Alert.alert(
          "Deposit successful",
          `Equivalent USD will be credited into your account. Thank you  🎉`,
          [
            {
              text: "Go Home",
              onPress: () => {
          ...
          render: () => (
            ...
            >
              <NativeBaseText style={{ color: "#FFFFFF" }}>
                {usdDepositResponse[usdDepositResponse.length - 1]?.message}
              </NativeBaseText>
            </Box>
          ),
        });
  ...
  const proceedUsdDeposit = () => {
    if (
      userUsdDepositDetails.payload.source == "" ||
      Number(userUsdDepositDetails.payload.amount) < 1000
    ) {
      ...
      <View style={{ margin: 10 }}>
        ...
        <View style={styles.creditBox}>
          {userUsdDepositDetails.payload.source !== "NGN" ? (
            <AppText>
              You will receive: $
              {userUsdDepositDetails.payload.amount > 0
                ? numberWithCommas(userUsdDepositDetails.payload.amount)
                : "0.00"}
            </AppText>
          ) : (
            <AppText>
              ...
              {userUsdDepositDetails.payload.amount > 0
                ? numberWithCommas(
                    userUsdDepositDetails.payload.amount /
                      Number(usdTransactions.ngn_usd_current)
                  )
                : "0.00"}
            </AppText>
          )}
