...
export default function Send({ navigation }) {
  ...
  const VETROPAY_FEE = Number(usdtTransactions?.vetropay_usdt_fee);
  ...
  useEffect(() => {
    if (recipientAddress != "" && postedTransactions.length && postedTransactions.length != prevPostedTransactions?.length) {
      if (postedTransactions[postedTransactions.length - 1]?.status == "success") {
        setDisplaySpinner(false);
        Alert.alert("Transfer successful", `${amount} USDT sent 🎉`, [
          {
            text: "Go Home",
            onPress: () => {
  ...
  const getTransactionFee = () => {
    if (recipientAddress.match(`T[A-Za-z1-9]{33}`) === null || recipientAddress == usdtTransactions?.address?.base58 || amount === "" || Number(amount) < 1) {
      let message;
      if (recipientAddress.match(`T[A-Za-z1-9]{33}`) === null) {
        message = "Invalid wallet address";
      } else if (recipientAddress == usdtTransactions?.address?.base58) {
      ...
      dispatch(getTrxFees("transfer", "USDT", recipientAddress));
    }
  };

  const completeUsdtTransfer = () => {
    if (transactionPin == "" || usdtTransactions?.balance?.balance < Number(amount) + Number(VETROPAY_FEE)) {
      let message;
      if (transactionPin === "") {
      ...
      // SEND TRANSACTION
      setDisplaySpinner(true);
      dispatch(postTrxTransaction("transfer", "USDT", recipientAddress, amount, transactionPin));
    }
  };
  return (
        ...
        <View style={{ margin: 10, justifyContent: "center", alignItems: "center" }}>
          ...
          <View style={styles.transferContainer}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <AppText styles={{ fontSize: 12 }}>${Number(VETROPAY_FEE).toFixed(2)}</AppText>
            </View>
            <HR style={{ marginVertical: 10, borderBottomColor: "grey" }} />
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              <AppText bold styles={{ color: "#36454F" }}>
              ...
              <AppText styles={{ fontSize: 12 }}>${(Number(fee.amount * TRON_VALUE_IN_USD) + Number(amount) + Number(VETROPAY_FEE)).toFixed(2)}</AppText>
            </View>
          </View>
