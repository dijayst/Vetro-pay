...
export default function Receive() {
  ...
  const copyToClipBoard = () => {
    Clipboard.setString(`${btcTransactions?.address?.address}`);
    toast.show({
      render: () => (
  ...
  const shareWalletAddress = async () => {
    try {
      const result = await Share.share({
        message: `My Wallet Address to receive Bitcoin (BTC):\n${btcTransactions?.address?.address}`,
      });
    } catch (error) {
      ...
      <View style={{ flex: 1, margin: 10 }}>
        <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
          <View style={styles.qrcodeContainer}>
            ...
            <QRCode value={`${btcTransactions?.address?.base58}`} size={200} bgColor="#FFFFFF" fgColor="#000000" getRef={(qrRef) => setQrRef(qrRef)} />

            <AppText styles={{ marginTop: 15, fontSize: 12 }}>{btcTransactions?.address?.address}</AppText>
          </View>
        </View>
      </View>
      <View style={{ height: 120, marginBottom: 10, paddingHorizontal: 10, justifyContent: "center", alignItems: "center" }}>
        <AppText>
          You can accept <AppText bold>Bitcoin (BTC)</AppText> via this address.
        </AppText>
